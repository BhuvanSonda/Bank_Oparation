...
def create_account():
    ...
    # Validate mobile number (should be exactly 10 digits, starting with 6-9)
        ...
        if mobile_number.isdigit():
            ...
            if not re.match("^[6-9]", mobile_number):
...
th=tk.PhotoImage(file="thumbsup.png")
...
tk.Button(button_frame, text="Deposit", command=deposit_amount,
          ...
          activebackground='blue', bg='#fc9342', fg='white',
          borderwidth=4, cursor="hand2").grid(row=1, column=1, padx=5, pady=5, sticky='ew')

tk.Button(button_frame, text="Withdraw", command=withdraw_amount,
          image=dbt_image, compound="left", font=("Arial", 12, 'bold'), height=60,
          activebackground='blue', bg='#c842fc', fg='white',
          borderwidth=4, cursor="hand2").grid(row=2, column=2, padx=5, pady=5, sticky='ew')

tk.Button(button_frame, text="Check A/C", command=check_account,
          image=chk_image, compound="left", font=("Arial", 12, 'bold'), height=60,
          activebackground='blue', bg='light blue', fg='white',
          borderwidth=4, cursor="hand2").grid(row=3, column=3, padx=5, pady=20, sticky='ew')

tk.Button(button_frame, text="Exit", bg='red', font=("Arial", 12, 'bold'), command=window.quit, cursor="hand2").grid(row=4, column=4, padx=5, pady=5, sticky='ew')
