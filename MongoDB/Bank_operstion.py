...
class Update:
   ...
   #function for assign values to the file
    def AC_entry(name, account_no, sign, init_balance,mobile_no,adhar_no,time,Ac_type):
        ...
        collection,client=Read_data()

        new_entry = {
            "Names": name,
            "A/C No:": int(account_no),
            ...
            "Time:": time,
            'AC_TYPE':str(Ac_type)}
        x=collection.insert_one(new_entry)
        
        client.close()
        print("Acknowledge : ",x.acknowledged,"inserted id : ",x.inserted_id)
...
class BankOperations:
    ...
    def ac_generator():
        ...
        for read in collection.find():
            if "A/C No:" in read and read["A/C No:"]:
 ...
 #function for deposit amount to the existing acount          
    def deposit(ac_no,amount):
        ...
        for read in collection.find():
            ...
            if ac_no == read["A/C No:"]:
                Amount=(read["Balance :"])+amount
                collection.update_one({"A/C No:":ac_no},{"$set":{"Balance :":Amount}})
...
#function for withdraw amount from the existing acount     
    def withdraw(ac_no,amount,sign,Ac_type):
        ...
        for read in collection.find():
                ...
                if ac_no == read["A/C No:"]:
                    if read["AC_TYPE"]==Ac_type:
                        if sign == read["Sign:"]:
                            if amount <= read["Balance :"]:
                                Amount=(read["Balance :"])-amount
                                collection.update_one({"A/C No:":ac_no},{"$set":{"Balance :":Amount}})
    ...
    def check(account_no):
        ...
        for read in collection.find():
                ...
                if account_no == read["A/C No:"]:
            
                    details=(f"\tAcount Details:\n\nName : {read['Names']}\n\nA/C No : {read[ 'A/C No:']}\n\nAvailable Balance : {read['Balance :']}\n\nMobile Number : {read['Mobile_No:']}\n\nAdhar Card Number : {read['Adhar_No:']}\n\nAc Type : {read['AC_TYPE']} ")
                    client.close()
                    return details
        else:
    ...
    def remove(ac_no,name,sign,adhar_no,mobile_no,ac_type):
        ...
        for read in collection.find():
            ...
            #Condition to check given details are valid or not
                    if read["Names"] == name:
                        if read["Sign:"] == sign:
                            if read["Mobile_No:"] == mobile_no:
                                if read["Adhar_No:"] == adhar_no:
                                    if  read["AC_TYPE"] == ac_type:
                                        collection.delete_one({"A/C No:":ac_no})
                                        client.close()
                                        return "removed"
                                    else:
                                        info="Ac_type_not_match"
...
def remove_valid(entries, toplevel=None, mobile_no=None, adhar_no=None, ac_no_var=None, name_var=None,
    ...
    read = Read_data()  # Function call to read CSV file
...
def check_valid(ac_no_entry, invalid_msg, entry, Toplevel):
    ...
    if not ac_no.isdigit():
        ...
        if details == "Ac_Number_not_exist":
            ...
            entry[0].focus_set()  # Set focus back to the entry field for user to re-enter the account number
        else:
                ...
                if read[ "Sign:"] == sign:
                    ...
                    if name:
                        if Validation.Name(name):  # Validate the name
                            ...
                            collection.update_one({"A/C No:":ac},{"$set":{"Names":name}})
                            updated = True
                        else:
                    ...
                    if new_sign:
                        if Validation.Sign(new_sign):  # Validate the new signature
                            ...
                            collection.update_one({"A/C No:":ac},{"$set":{"Sign:":new_sign}})
                            updated = True
                    ...
                    if mobile_no:
                        if Validation.Mobile(mobile_no):  # Validate the mobile number
                            ...
                            collection.update_one({"A/C No:":ac},{"$set":{"Mobile_No:":int(mobile_no)}})
                            updated = True
                        else:
...
def load_csv(Secret_key_Entry, label, button, invalid, tree, event=None):
    ...
    if enter_key != Secret_key:
        ...
        for heading in headers:
            data.heading(heading, text=heading, anchor='nw')  

           
        for document in documents:
            tree.insert("", "end", values=[str(document.get(header, "")) for header in headers])
