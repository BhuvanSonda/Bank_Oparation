...
#assign a file
Acounts_file='data.xlsx'

AC_close_file='Closed_Acounts.xlsx'
ac_closed = pd.read_excel(AC_close_file) if pd.io.common.file_exists(AC_close_file) else pd.DataFrame(columns=["Names", "A/C No:", "Sign:","Mobile_No:",'Adhar_No:',"Time:","Reason"])
...
def Read_Excel():
    ...
    if os.path.isfile(Acounts_file):
        # Read the existing Excel file
        read = pd.read_excel(Acounts_file)
    else:
        # Return an empty DataFrame with the specified columns
        read = pd.DataFrame(columns=["Names", "A/C No:", "Balance:", "Sign:", "Mobile_No:", "Adhar_No:", "Time:", "AC_TYPE"])
    return read
...
class Update:
    ...
    def AC_entry(name, account_no, sign, init_balance, mobile_no, adhar_no, time, ac_type):
        ...
        if os.path.isfile(Acounts_file):
            with pd.ExcelWriter(Acounts_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
    ...
    def closed(name, account_no, sign, mobile_no, adhar_no, time, reason):
        ...
        # Append to the file if it exists
        if os.path.isfile(AC_close_file):
            with pd.ExcelWriter(AC_close_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                df.to_excel(writer, index=False, header=False, startrow=writer.sheets['Sheet1'].max_row)
        else:
            with pd.ExcelWriter(AC_close_file, engine='openpyxl', mode='w') as writer:
                df.to_excel(writer, index=False)
...
def remove_valid(entries, toplevel=None, mobile_no=None, adhar_no=None, ac_no_var=None, name_var=None,
    ...
    read = Read_Excel()  # Function call to read CSV file
    ...
    if a == 1:
        ...
        if details == "removed":
            ...
            Update.closed(name, ac, sign, mobile_no=mobile_no, adhar_no=adhar_no, time=time, reason=reason)  # Update the closed account details
