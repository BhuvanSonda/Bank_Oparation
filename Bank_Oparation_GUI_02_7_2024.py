...
#assign a file
Acounts='data_varify.csv'

#function to read the file
def Read_csv():
    read = pd.read_csv(Acounts) if pd.io.common.file_exists(Acounts) else pd.DataFrame(columns=["Names", "A/C No:", "Balance :", "Sign:","Mobile_No:",'Adhar_No:',"Time:"])
    return read
...
class Update:
   ...
   #function for assign values to the file
    def update(name, account_no, sign, init_balance,mobile_no,adhar_no,time):
        ...
        if pd.io.common.file_exists(Acounts):
            df.to_csv(Acounts, mode="a", index=False, header=False)
        else:
            df.to_csv(Acounts, mode="w", index=False)
 ...
 #function for deposit amount to the existing acount          
    def deposit(ac_no,amount):
        ...
        if ac_no in read["A/C No:"].values:
            ...
            read.to_csv(Acounts, index=False)
...
def focus_next_entry(event, entries,mobile_var=None,adhar_var=None,amount_var=None):
    ...
    entries[next_index].focus_set()#subimit.focus_set()
    
    # if len(entries)>=5:
    if len(entries)>=4:
        # create_valid(entries,mobile_var,adhar_var,amount_var)
        create_valid_fun_res = create_valid(entries,mobile_var,adhar_var,amount_var)
        print("create_valid_fun_res:------", create_valid_fun_res)
    elif len(entries)>=3:
        print("***********entries >= 3***********")
        withdraw_valid(entries,amount_var)
    elif len(entries)==2:
        print("***********entries == 2***********")
        deposit_valid(entries,amount_var)
 
    return 'break'

#valid checkup for create ac
def create_valid(entries,mobile_var=None,adhar_var=None,amount_var=None):
    global a
    a=1
    amt = amount_var.get()
        #print("AMT:----", amt)
...
def deposit_valid(entries,amount_var=None):
    global a
    a=1
    amt=amount_var.get()
...
# Bind the Enter key event to move focus
    ...
    submit_btn.grid(row=6,column=0,padx=10,pady=10,columnspan=2)
...
# Bind the Enter key event to move focus
    for entry in entries:
        entry.bind("<Return>", lambda event, entries=entries: focus_next_entry(event, entries,amount_var=amount_var))
...
def withdraw_amount():
    ...
    def validate_inputs():
        ...
        # Validate amount (should be positive numbers)
        # if not Validation.Amount(amount):
        #     amount_entry.config(fg='red')
        #     all_valid = False
        # else:
        #     amount_entry.config(fg='black')
    ...
    amount_entry.bind("<KeyRelease>", lambda event: validate_inputs())
       ...
       # to create lables and entry box
    tk.Label(mini, text='Enter Account Number:', font=("Arial", 10)).grid(row=2, column=0, padx=10, pady=10)
    ac_no_entry = tk.Entry(mini, width=30)
    ac_no_entry.grid(row=2, column=1, padx=10, pady=10)
    ac_no_entry.bind("<KeyRelease>", lambda event: validate_inputs())
    ...
    entries = [ amount_entry,ac_no_entry, sign_entry, ]
...
def check_account():
    ...
    submit_btn=tk.Button(mini,text='SUBMIT',font=("Arial",10),command=Submit,state="disabled")
    submit_btn.grid(row=2,column=0,padx=10,pady=10,columnspan=2)
